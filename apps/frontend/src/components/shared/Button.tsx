...
export function Button ({ children, onClick, disabled, variant = 'primary', className, type = 'button' }: ButtonProps): React.ReactElement {
  ...
  return (
    <button onClick={onClick} disabled={disabled} className={cn(baseStyles, colors, className)} type={type}>
      {children}
    </button>
  )
}
